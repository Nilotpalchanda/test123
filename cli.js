...
class AIProjectUpdater {
  constructor() {
    ...
    this.config = {
      packagesToRemove: ['@abc/tailwind-config', '@abc/typescript-utils'],
      packagesToAdd: ['@tailwindcss/postcss', '@abc/design-system@2.0.0-beta.0'],
      filesToEdit: [],
      filesToCreate: [
        {
        ...
        },
        {
          ...
          content: `@import 'tailwindcss';\n@import '../../node_modules/@vialto/design-system/dist/globals.css';`
        }
      ]
    };
      ...
      try {
        ...
        if (fs.existsSync(packageJsonPath)) {
          ...
          if (packageJson.dependencies?.[pkg] || packageJson.devDependencies?.[pkg]) {
            ...
            execSync(`pnpm remove ${pkg}`, { stdio: 'pipe' });
            this.success(`Removed ${pkg}`);
            this.stats.packagesRemoved++;
          } else {
            this.info(`Package ${pkg} not found in package.json, skipping`);
          }
        }
      } catch (error) {
    ...
    for (const pkg of this.config.packagesToAdd) {
      ...
      try {
        ...
        if (pkg === '@tailwindcss/postcss') {
          execSync(`pnpm install ${pkg} --save-dev`, { stdio: 'pipe' });
        } else {
          execSync(`pnpm install ${pkg} --save`, { stdio: 'pipe' });
        }
...
export default AIGeneratedComponent;`;
  ...
  async run() {
    try {
      ...
      // Remove tailwind.config.js if it exists
      if (fs.existsSync('tailwind.config.js')) {
        await this.showSpinner('Removing tailwind.config.js...', 1000);
        fs.unlinkSync('tailwind.config.js');
        this.success('Removed tailwind.config.js');
      }

      // Rename postcss.config.js to postcss.config.mjs if it exists
      if (fs.existsSync('postcss.config.js')) {
        await this.showSpinner('Renaming postcss.config.js to postcss.config.mjs...', 1000);
        fs.renameSync('postcss.config.js', 'postcss.config.mjs');
        this.success('Renamed postcss.config.js to postcss.config.mjs');
      }
